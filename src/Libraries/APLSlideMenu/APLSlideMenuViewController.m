...
- (UIView *)APLSlideMenuViewController_findSubviewOfClass:(Class)class {
    ...
    for (UIView *view in self.subviews) {
        UIView *foundSubview =
            [view APLSlideMenuViewController_findSubviewOfClass:class];
        if (foundSubview) {
            return foundSubview;
        }
    }
...
NSString *APLSlideMenuWillShowNotification =
    @"APLSlideMenuWillShowNotificationInternal";
NSString *APLSlideMenuDidShowNotification =
    @"APLSlideMenuDidShowNotificationInternal";
NSString *APLSlideMenuWillHideNotification =
    @"APLSlideMenuWillHideNotificationInternal";
NSString *APLSlideMenuDidHideNotification =
    @"APLSlideMenuDidHideNotificationInternal";
...
@interface APLSlideMenuViewController (
...
@property (nonatomic, assign, getter=isDisplayMenuSideBySide)
    BOOL displayMenuSideBySide;
@property (nonatomic, strong, readwrite)
    UIViewController *activeMenuViewController;
...
- (void)viewDidLoad {
    ...
    UIPanGestureRecognizer *panGR = [[UIPanGestureRecognizer alloc]
        initWithTarget:self
                action:@selector(dragGestureRecognizerDrag:)];
    panGR.delegate                = self;
    ...
    self.leftSeparatorView                  = [[UIView alloc]
        initWithFrame:CGRectMake(0., 0., 1., self.view.bounds.size.height)];
    self.leftSeparatorView.backgroundColor  = self.separatorColor;
    self.leftSeparatorView.autoresizingMask = UIViewAutoresizingFlexibleHeight
        | UIViewAutoresizingFlexibleRightMargin;
    self.leftSeparatorView.hidden = YES;
    [contentContainer addSubview:self.leftSeparatorView];

    self.rightSeparatorView =
    ...
    self.rightSeparatorView.autoresizingMask =
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!self.presentedViewController) {
        [self displayMenuSideBySideIfNeededForOrientation:
...
- (void)updateShadowPath {
    ...
    currentView.layer.shadowPath =
        [UIBezierPath bezierPathWithRect:currentView.bounds].CGPath;
}
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation
    ...
    return (self.contentViewController != nil)
        ? [[self getTopContentViewController]
...
- (BOOL)shouldAutorotate {
    return (self.contentViewController != nil)
        ? [[self getTopContentViewController] shouldAutorotate]
        : YES;
}

- (NSUInteger)supportedInterfaceOrientations {
    return (self.contentViewController != nil)
        ? [[self getTopContentViewController] supportedInterfaceOrientations]
...
- (void)displayMenuSideBySideIfNeededForOrientation:(UIInterfaceOrientation
                                                    )orientation {
    BOOL displayMenuSideBySide =
        (self.isShowLeftMenuInLandscape || self.isShowRightMenuInLandscape)
        && (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        && UIInterfaceOrientationIsLandscape(orientation);

    CGFloat offsetLeft = displayMenuSideBySide && self.isShowLeftMenuInLandscape
        ? [self menuAbsoluteWidth]
        : 0.;
    CGFloat offsetRight =
        displayMenuSideBySide && self.isShowRightMenuInLandscape
        ? [self menuAbsoluteWidth]
        : 0.;
    ...
    self.leftSeparatorView.hidden =
        !(displayMenuSideBySide && self.isShowLeftMenuInLandscape);
    self.rightSeparatorView.hidden =
        !(displayMenuSideBySide && self.isShowRightMenuInLandscape);

    self.contentContainerView.clipsToBounds =
        displayMenuSideBySide || !self.useShadow;
    self.displayMenuSideBySide = displayMenuSideBySide;
    self.leftMenuViewController.view.accessibilityElementsHidden =
        !displayMenuSideBySide || !self.isShowLeftMenuInLandscape;
    self.rightMenuViewController.view.accessibilityElementsHidden =
        !displayMenuSideBySide || !self.isShowRightMenuInLandscape;
}
...
- (UIViewController *)childViewControllerForStatusBarStyle {
    return self.isMenuViewVisible ? self.activeMenuViewController
...
- (CGFloat)menuAbsoluteWidth {
    return (self.menuWidth >= 0.0 && self.menuWidth <= 1.0)
        ? roundf(self.menuWidth * self.view.bounds.size.width)
        : self.menuWidth;
}

- (UITapGestureRecognizer *)hideTapGestureRecognizer {
    if (_hideTapGestureRecognizer == nil) {
        _hideTapGestureRecognizer          = [[UITapGestureRecognizer alloc]
            initWithTarget:self
                    action:@selector(closeTapGestureRecognizerFired:)];
...
- (void)setLeftMenuViewController:(UIViewController *)menuViewController {
    ...
    if (menuViewController) {
        ...
        menuFrame.size.width =
            self.menuAbsoluteWidth + kAPLSlideMenuFirstOffset;
...
- (void)setRightMenuViewController:(UIViewController *)menuViewController {
    ...
    if (menuViewController) {
        ...
        menuView.frame            = menuFrame;
        menuView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin
            | UIViewAutoresizingFlexibleHeight;
...
- (void)setGestureSupport:(APLSlideMenuGestureSupportType)gestureSupport {
    ...
    self.dragGestureRecognizer.enabled =
        gestureSupport != APLSlideMenuGestureSupportNone;
}
...
- (void)setUseShadow:(BOOL)useShadow {
    ...
    self.contentContainerView.clipsToBounds =
        !useShadow || self.isDisplayMenuSideBySide;
}
...
- (void)setContentViewController:(UIViewController *)contentViewController
    ...
    [self.contentContainerView insertSubview:contentViewController.view
                                     atIndex:0];
    ...
    if (animated) {
        [UIView animateWithDuration:kAPLSlideMenuDefaultAnimationDuration
...
- (void)dismissContentViewController {
    ...
    // Reset visibility because the content view controller should be moved if
    // setContentViewController is called.
...
- (void)navigationController:(UINavigationController *)navigationController
       didShowViewController:(UIViewController *)viewController
                    animated:(BOOL)animated {
    // hack to trigger rotation to supported orientation in case this is not the
    // topViewController
...
- (void)addShadowToView:(UIView *)aView {
    ...
    aView.layer.shadowPath =
        [UIBezierPath bezierPathWithRect:aView.bounds].CGPath;
...
- (BOOL)delegateVetosDragWithStartPoint:(CGPoint)startPoint
                               endPoint:(CGPoint)endPoint
                      inOrderToShowMenu:
                          (BOOL)isGoingToShowMenuWhenThisGestureSucceeds {
    if ([self.slideDelegate
            respondsToSelector:@selector
            (shouldHandleSlideMenuGesture:
                          startingAtPoint:withEndPoint:inOrderToShowMenu:)]) {
        return ![self.slideDelegate
            shouldHandleSlideMenuGesture:self
                         startingAtPoint:startPoint
                            withEndPoint:endPoint
                       inOrderToShowMenu:
                           isGoingToShowMenuWhenThisGestureSucceeds];
    }
    return NO;
}
    ...
    switch (state) {
        ...
        case UIGestureRecognizerStateChanged: {
            if (self.gestureSupport == APLSlideMenuGestureSupportDrag
                ...
                CGFloat endX = (self.menuWidth >= 0.0 && self.menuWidth <= 1.0)
                    ? roundf(self.menuWidth * contentFrame.size.width)
                    : self.menuWidth;

                newStartX = MIN(
                    MAX(self.rightMenuViewController ? -endX : 0., newStartX),
                    self.leftMenuViewController ? endX : 0.
                );
                if (newStartX < 0) {
                    [self.view
                        sendSubviewToBack:self.leftMenuViewController.view];
                } else {
                    [self.view
                        sendSubviewToBack:self.rightMenuViewController.view];
                }
        ...
        case UIGestureRecognizerStateEnded: {
            if (self.gestureSupport == APLSlideMenuGestureSupportBasic
                || self.gestureSupport
                    == APLSlideMenuGestureSupportBasicOnlyHorizontal) {
                BOOL hasCorrectAngle = YES;
                if (self.gestureSupport
                    == APLSlideMenuGestureSupportBasicOnlyHorizontal) {
                ...
                /*
                 * Spaghetti code ahead. Grab yourself some tomatoe sauce and
                 * enjoy. Yes, there are these many different cases.
                 */
                if (xTranslation > 0.0 && hasCorrectAngle) {
                    if (self.leftMenuViewController
                        && !self.isMenuViewVisible) {
                        if (![self delegateVetosDragWithStartPoint:
                                       self.dragGestureStartPoint
                                                          endPoint:endPoint
                                                 inOrderToShowMenu:YES]) {
                ...
                if (((0. < currentX)
                     && (flickVelocitiy <
                         [sender velocityInView:self.contentContainerView].x))
                    || ((currentX < 0.)
                        && ([sender velocityInView:self.contentContainerView].x
                            < -flickVelocitiy))) {
                    if (xTranslation > 0.0) {
                    ...
                    CGFloat endX =
                        (self.menuWidth >= 0.0 && self.menuWidth <= 1.0)
                        ? roundf(self.menuWidth * contentFrame.size.width)
                        : self.menuWidth;
...
- (void)notifyWillShowMenu {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:APLSlideMenuWillShowNotification
                      object:self];
}

- (void)notifyDidShowMenu {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:APLSlideMenuDidShowNotification
                      object:self];
}

- (void)notifyWillHideMenu {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:APLSlideMenuWillHideNotification
                      object:self];
}

- (void)notifyDidHideMenu {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:APLSlideMenuDidHideNotification
                      object:self];
}
...
- (void)showMenu:(UIViewController *)viewController animated:(BOOL)animated {
    if (self.isDisplayMenuSideBySide) {
        return;
    }

    CGFloat direction =
        (viewController == self.leftMenuViewController) ? 1. : -1.;

    viewController.view.accessibilityElementsHidden = NO;
    UIAccessibilityPostNotification(
        UIAccessibilityScreenChangedNotification, viewController.view
    );
    ...
    void (^showMenuCompletionBlock)(BOOL) = ^(BOOL finished) {
        ...
        if (self.tapOnContentViewToHideMenu) {
            [self.contentContainerView
                addGestureRecognizer:self.hideTapGestureRecognizer];
            self.contentViewController.view.userInteractionEnabled = NO;
        }
        if (self.slideDelegate &&
            [self.slideDelegate respondsToSelector:@selector(didShowMenu:)]) {
    ...
    void (^showBouncingBlock)() = ^{
        ...
        __block CGFloat endx        = direction
            * ((self.menuWidth >= 0.0 && self.menuWidth <= 1.0)
                   ? roundf(self.menuWidth * contentFrame.size.width)
                   : self.menuWidth);
    ...
    void (^showMenuBlock)() = ^{
        ...
        contentFrame.origin.x = direction
            * ((self.menuWidth >= 0.0 && self.menuWidth <= 1.0)
                   ? roundf(self.menuWidth * contentFrame.size.width)
                   : self.menuWidth);
        self.contentContainerView.frame = contentFrame;
    ...
    if (self.slideDelegate &&
        [self.slideDelegate respondsToSelector:@selector(willShowMenu:)]) {
...
- (void)hideMenu:(BOOL)animated {
    ...
    CGFloat direction =
        (self.activeMenuViewController == self.leftMenuViewController) ? 1.
                                                                       : -1.;
    UIAccessibilityPostNotification(
    ...
    void (^hideMenuCompletionBlock)(BOOL) = ^(BOOL finished) {
        ...
        if (self.tapOnContentViewToHideMenu) {
            [self.contentContainerView
                removeGestureRecognizer:self.hideTapGestureRecognizer];
            self.contentViewController.view.userInteractionEnabled = YES;
        }

        if (self.slideDelegate &&
            [self.slideDelegate respondsToSelector:@selector(didHideMenu:)]) {
    ...
    if (self.slideDelegate &&
        [self.slideDelegate respondsToSelector:@selector(willHideMenu:)]) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (self.gestureSupport
        == APLSlideMenuGestureSupportDragOnlyNavigationBar) {
        UIView *navigationBar = [self.contentContainerView
            APLSlideMenuViewController_findSubviewOfClass:[UINavigationBar
                                                              class]];
        if (!CGRectContainsPoint(
                navigationBar.bounds,
                [gestureRecognizer locationInView:navigationBar]
            )) {
            return NO;
        }
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
    ...
    return (gestureRecognizer != self.dragGestureRecognizer)
...
- (APLSlideMenuViewController *)slideMenuController {
    ...
    while ((currentViewController = currentViewController.parentViewController)
        ...
        if ([currentViewController
                isKindOfClass:[APLSlideMenuViewController class]]) {
