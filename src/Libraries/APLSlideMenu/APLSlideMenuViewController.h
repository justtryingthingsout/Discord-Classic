...
typedef NS_ENUM(NSInteger, APLSlideMenuGestureSupportType) {
    ...
    APLSlideMenuGestureSupportBasicOnlyHorizontal // Only register horizontal
                                                  // drags
};
...
@optional
- (BOOL
)shouldHandleSlideMenuGesture:(APLSlideMenuViewController *)aViewController
              startingAtPoint:(CGPoint)startPoint
                 withEndPoint:(CGPoint)endPoint
            inOrderToShowMenu:(BOOL)isGoingToShowMenuWhenThisGestureSucceeds;
...
@property (nonatomic, strong)
    IBOutlet UIViewController *rightMenuViewController;
@property (nonatomic, strong) IBOutlet UIViewController *contentViewController;

/** Listen to show and hide events. */
@property (nonatomic, weak) id<APLSlideMenuViewControllerDelegate>
    slideDelegate;
...
/** Width of menu. 0.0 - 1.0 are interpreted as relative width, all other as
 * absolute width. */
...
@property (nonatomic, assign, getter=isShowLeftMenuInLandscape)
    BOOL showLeftMenuInLandscape;
@property (nonatomic, assign, getter=isShowRightMenuInLandscape)
    BOOL showRightMenuInLandscape;

/** draw a shadow above menu view controllers, default is YES */
@property (nonatomic, assign) BOOL useShadow;

/** color of border between menu and content in landscape on iPad when
 * showLeftMenuInLandscape or showRightMenuInLandscape is enabled, default is
 * [UIColor clearColor] */
