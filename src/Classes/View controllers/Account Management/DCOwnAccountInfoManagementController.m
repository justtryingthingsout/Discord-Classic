...
- (void)viewDidLoad
{
    ...
    NSDictionary *userInfo = [DCServerCommunicator.sharedInstance currentUserInfo];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString* avatarURL = [NSString stringWithFormat:@"https://cdn.discordapp.com/avatars/%@/%@.png?size=64", [userInfo objectForKey:@"id"], [userInfo objectForKey:@"avatar"]];
        
        [DCTools processImageDataWithURLString:avatarURL andBlock:^(UIImage *imageData){
            UIImage *retrievedImage = imageData;
            dispatch_async(dispatch_get_main_queue(), ^{
                self.pfp.image = retrievedImage;
                self.pfp.layer.cornerRadius = self.pfp.frame.size.width / 2.0;
                self.pfp.layer.masksToBounds = YES;
            });
        }];
    });
    
    if ([userInfo objectForKey:@"banner"] && ![[userInfo objectForKey:@"banner"] isKindOfClass:[NSNull class]]) {
        ...
        if (![[userInfo objectForKey:@"banner_color"] isKindOfClass:[NSNull class]]) {
            UIColor *backgroundColor = [UIColorHex colorWithHexString:[userInfo objectForKey:@"banner_color"]];
            if (backgroundColor) {
                dispatch_async(dispatch_get_main_queue(), ^{
